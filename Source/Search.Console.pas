...
**)
Function TSearchConsole.BackGroundColour(Const iColour, iNone : TColor) : Integer;
...
**)
Procedure TSearchConsole.CheckForEscape;
...
**)
Procedure TSearchConsole.ClearLine;
...
Begin
  ...
  OutputToConsole(coStd, StringOfChar(#32, ConsoleInfo.dwSize.X -
    ConsoleInfo.dwCursorPosition.X - 1), TSearchColour.scNone, TSearchColour.scNone, False);
End;
...
**)
Constructor TSearchConsole.Create;
...
**)
Function TSearchConsole.ForeGroundColour(Const iColour, iNone : TColor): Integer;
...
(**
  ...
  @precon  None.
  @postcon Returns the colour associated with the given enumerate.

  @param   eColour as a TSearchColour as a constant
  @return  a TColor
...
**)
Function TSearchConsole.GetConsoleCharacter(Const Characters: TSysCharSet): Char;
...
**)
Function TSearchConsole.GetErrHnd: NativeUInt;
...
**)
Function TSearchConsole.GetStdHnd: NativeUInt;
...
**)
Function TSearchConsole.GetWidth: Integer;
...
(**
  ...
  This method outputs a message on the screen asking the user if they wish to stop processing.
  If the user pressews Y then the application is terminated through an Abort exception.

  @precon  None.
  @postcon If the user presses Y Abort is raised to stop the application.

**)
Procedure TSearchConsole.ProcessEscapeKey;
...
Begin
  ...
  OldPos := ConsoleInfo.dwCursorPosition;
  ClearLine;
  OutputToConsole(coStd, strMsg, scInput);
  C := GetConsoleCharacter(['y', 'Y', 'n', 'N']);
  Case C Of
    'y', 'Y':
      Begin
        OutputToConsoleLn(coStd, strYes, scInput);
        Abort
      End;
    'n', 'N':
      Begin
...
(**
  ...
  @precon  None.
  @postcon Sets the colour of the given enumerated item.
